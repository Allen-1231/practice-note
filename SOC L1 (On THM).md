...
- alert reporting：在關閉警報或上報至二級分析師之前，需要先提交報告。可能需要詳細記錄調查過程，確保包含所有相關證據，而不是僅提供簡短的警報評論。
    ...
    - 使用 5W (Who, What, When, Where, Why) 撰寫報告。
        - 範例：
        ![image](https://hackmd.io/_uploads/ByK83JSUbg.png)
        - 練習：
        ![image](https://hackmd.io/_uploads/BJ42CJH8bg.png)
...
![螢幕擷取畫面 2026-05-18 122724](https://hackmd.io/_uploads/Ska2LX_kzx.png)
...
來自組織網路外部，並掃描網絡內部的機器 (主要針對網絡邊緣的公共資產) 的掃描活動。這種攻擊的來源 IP 是外部 IP 地址，目標 IP 為屬於被攻擊組織的 IP 地址。
    ...
    ![image](https://hackmd.io/_uploads/Hk5kjHKkGg.png)

    這種類型的掃描是攻擊的初期階段，攻擊者尚未在網絡中建立任何可利用的入口，因此屬於低危險等級的警報，分析師可以阻擋組織網路邊界防火牆上的惡意 IP 地址，然而攻擊者也有可能隱藏其 IP 地址再次發起攻擊。
...
內部網路到內部網路的掃描活動，來源 IP 與目標 IP 都是組織內網中的私有 IP 地址。
    ...
    ![image](https://hackmd.io/_uploads/SkTuaHF1Mg.png)
...
![螢幕擷取畫面 2026-05-19 200042](https://hackmd.io/_uploads/HJcOFRFkfg.png)
![螢幕擷取畫面 2026-05-19 200140](https://hackmd.io/_uploads/HykYYAKkGl.png)

1. Which file contains logs that showcase internal scanning activity?
![螢幕擷取畫面 2026-05-19 201738](https://hackmd.io/_uploads/Hk9IKRKJGl.png)
...
![螢幕擷取畫面 2026-05-19 203422](https://hackmd.io/_uploads/SJQNKRt1Ge.png)
A：2276

3. What is the external IP address that is performing external scanning activity?
![image](https://hackmd.io/_uploads/ryMN39qyGl.png)
A：203.0.113.25
...
一旦攻擊者知道網絡中有哪些主機存在，他們通常會嘗試確定這些主機上哪些端口是有開放的，此行為稱作端口掃描 (port scan)，並且分為以下兩種：
- Horizontal Scanning

    掃描多個目標 IP 地址上的同一端口，即屬於水平掃描。水平掃描用於確定哪些主機暴露了特定的端口。如果攻擊者打算利用該特定端口，他們可能會進行水平掃描。例如：WannaCry 勒索軟體透過利用 SMBv1 漏洞在網路中傳開，並掃描 port 445 (用於 SMB 的 port) 為開放的主機。
    
    如果在日誌中發現到多個事件中，一樣的來源 IP (src_ip)、單一的目標端口 (dst_port)、但多種目標 IP (dst_ip)，則可以判斷為水平掃描 (Horizontal Scanning)。

    ![image](https://hackmd.io/_uploads/SJ9fLrjkGe.png)

- Vertical Scanning
    
    垂直掃描指在單一主機 IP 地址上掃描多個端口。攻擊者會利用此掃描方式在主機中建立立足點，並識別主機中暴露的服務。通常發生在攻擊者認為該主機對於他們的目的而言很有價值，因此專注在這單一機器上試著找到弱點。
    
    如果在日誌中發現到多個事件中，一樣的來源 IP (src_ip)、一樣的目標 IP (dst_ip)、但多種目標端口 (dst_port)，則可以判斷為垂直掃描 (Vertical Scanning)
    
    ![image](https://hackmd.io/_uploads/S18qpknJfe.png)
    
在某些情況下，攻擊者可能會同時進行水平和垂直掃描，以結合這兩種掃描方法的優勢。

練習：
1. One of the log files contains evidence of a horizontal scan. Which IP range was scanned? Format X.X.X.X/X
![image](https://hackmd.io/_uploads/HJpV4e2JGx.png)
只有 `log-session-2.csv` 有不同目標 IP。
這只是大概看前幾行，更準確的看全部：
![image](https://hackmd.io/_uploads/HkNfBe31Me.png)
0 與 1 分別都有一個目標 IP 不同，其餘皆同一個目標 IP，由圖可確認具有水平掃描特徵的是 `log-session-2.csv` 這個檔案。
    
    ![image](https://hackmd.io/_uploads/Bytaux21Gx.png)
    ...
    ![image](https://hackmd.io/_uploads/SJ00dlhyzx.png)

    `192.168.230.X` 的紀錄相當多次，我們要的是範圍廣、每個都掃一次的 `203.0.113.X`。
    
    網路範圍為 `203.0.113.2` 到 `203.0.113.254`。
    
    ```
    CIDR 表示法  固定的位元  剩下的主機位元  總共包含的 IP 數
       /16        前16位    16位 (32-16)    65536 (2^16)
       /24        前24位     8位 (32-24)      256 (2^8)
       /25        前25位     7位 (32-25)      128 (2^7)
       /26        前26位     6位 (32-26)       64 (2^6)
       /30        前30位     2位 (32-30)        4 (2^4)
       
    192.168.1.0/26 相當於這個網段一共有 64 個 IP，大網路會被切成：
        第一組：192.168.1.0 ~ 192.168.1.63
        第二組：192.168.1.64 ~ 192.168.1.127
        第三組：192.168.1.128 ~ 192.168.1.191
        第四組：192.168.1.192 ~ 192.168.1.255
    若某台電腦 IP 為 192.168.1.70/26，即屬於第二組，而它只能與屬於第二組的通訊，與其他網段要通訊需要透過路由器。
    ```
...
2. In the same log file, there is one IP address on which a vertical scan is performed. Which IP address is this?
    ...
    有相當多筆紀錄的 `192.168.230.X` 自然就是垂直掃描的特徵：
    ![image](https://hackmd.io/_uploads/SyHo5g2kGe.png)
    A：192.168.230.145
...
3. Is there any UDP scanning attempt in the logs? Y/N
    ...
    代表有 UDP 掃描的跡象：
    - 同一 src_ip 向多個 dst_port 發送 UDP 封包 (垂直 UDP 掃描)。
    - 同一 src_ip 向多個 dst_ip 發送 UDP 封包 (水平 UDP 掃描)。
    - 大量的同一 src_ip 針對多個 port/主機，且大量收到 `ICMP port unreachable` 的回應。
    
    圖中可看到只有兩條紀錄，極少量，沒有反覆向多個目標或多個端口發送流量的情況，因此判定無 UDP 掃描。
    A：`N`
...
在默認啟用的所有 Windows 日誌中，安全事件日誌（Security event log）能提供最大的價值。

|    Event ID     | 目的                                                               | 日誌紀錄位置                               | 限制                                               |
|:---------------:|:------------------------------------------------------------------ |:------------------------------------------ |:-------------------------------------------------- |
| 4624 (登入成功) | 偵測可疑的遠端連線 (RDP)/網路 (network) 的登入，並識別攻擊的起點。 | 紀錄在被嘗試取得存取的那台受攻擊的主機上。 | 雜訊多，伺服器每分鐘就會有上百次登入事件。         |
...
- New Logon: 真正成功登入的使用者帳號。

    `Account Name`: 登入的使用者名稱。

    `Logon ID`: 該次登入會話的獨特識別碼，可以用此 ID 去連結該使用者登入後做了其它哪些事。

- Logon Information: `Logon Type` 為登入類型，可用來判斷攻擊手法。數字代表使用者的登入管道，例如：

    10 是 RDP (透過遠端桌面連線登入)。

    3 是 network (存取共享資料夾 (SMB)、連線資料庫或內部服務)，密碼噴灑攻擊成功時，常出現此類型。

    2 是本地互動式登入，使用者在電腦前敲鍵盤登入。

- Network Information: 當 Logon Type 是 3 或 10 時，這個區塊才有價值：

    `Workstation Name`: 來源電腦主機名稱。(主機名稱很容易被惡意偽造，不可完全信任，需以 IP 為準)

    `Source Network Address`: 來源 IP 位址。最可靠的證據，可以指出攻擊者是從內部哪台被淪陷的電腦，或是外部哪個 IP 連過來的。

---

偵測 RDP 暴力破解：
1. 打開安全日誌並篩選 event ID 為 4625 登入失敗的事件。

2. 尋找 Logon Type 為 3 或 10 的事件。

    對於大多數現代系統，登入類型通常為 3，因為 NLA 預設是啟用的。然而較老舊或配置錯誤的系統，登入類型通常為 10。
    
3. 需要警惕的情況：

    嘗試登入的使用者很多為 admin、helpdesk、cctv：表示密碼噴灑攻擊。
    
    同一帳號有多次登入失敗，通常使用者帳戶為 administrator：表示暴力破解攻擊。
...
分析 RDP 登入：
1. 打開安全日誌並篩選 event ID 為 4624 登入成功的事件。

2. 尋找 Logon Type 為 10 的事件。

    如果 NLA 已啟用，所有 RDP 登入事件都被歸於 logon type 為 3 的 4624。
    
    要獲得一個真實的工作站名稱，需要檢查之前的登入類型為 3 的事件。
    
3. 警示代表之前的暴力破解攻擊，或是可疑的來源 IP/主機名。

4. 如果假設登入確實是惡意行為：

    Windows 會分配一個 logon ID 給每個成功登入的事件。此 ID 是獨一無二的，儲存便於往後分析用。
...
|      Event ID      | 說明                                                                          | 惡意用途                                                                   |
|:------------------:|:----------------------------------------------------------------------------- |:-------------------------------------------------------------------------- |
| 4720 / 4722 / 4738 | 一個使用者帳號被 建立 created / 啟用 enabled / 改變 changed。                 | 攻擊者可能會建立後門帳號或甚至啟用舊的，來避免被偵測到。                   |
|    4725 / 4726     | 一個使用者帳號被 停用 disabled / 刪除 deleted。                               | 攻擊者可能會停用 SOC 的特權帳號，來減緩 SOC 的行動。                       |
|    4723 / 4724     | 一位使用者更改了他們的密碼 / 使用者的密碼被重置。                             | 如果攻擊者獲得了足夠的權限，他們可能會重置密碼，然後存取需要的使用者帳戶。 |
|    4732 / 4733     | 一位使用者被新增到 added to / 移出 removed from 一個安全群組 security group。 | 攻擊者通常會將他們的後門帳號加進特權群組，像是 Administrators 管理員。     |

User Management Events 使用者管理事件的結構：

可分為三個部分，誰做了此動作 (Subject)、誰是目標 (Object)、具體的變動細節 (Details)：
1. Subject: 執行動作的帳號。(其中有與 4624 一樣的 Logon ID 欄位資訊)

2. Object: 動作的目標。名稱會根據 Event ID (例如新帳號或新成員) 不同而有不同的命名。

3. Details: 此操作改變了什麼具體內容，細節會根據 Event ID 不同而有不同的操作細節。例如：4732 / 4733 的細節指哪個目標群組被更動了 (加入 / 移出)；4720 的細節指創建的新帳號的各項屬性設定。

    ![image](https://hackmd.io/_uploads/r1P63XOlzg.png)
    
尋找被攻擊者當作後門的使用者帳號 (Backdoored Users)：
1. 打開安全日誌，篩選 event ID 為 4720 與 4732 的事件。

2. 手動檢查每個事件，危險訊號是：
    
    - 自己的 IT 部門中沒有人能證實該操作。
    - 變更動作發生的時間是在非工作時間或週末進行的。
    - Subject 使用者名稱是未知的。
    - 目標使用者的名稱不是常見的命名模式。

3. 若確認該操作是惡意的，找出登入細節：

    - 從事件 4720 /4732 複製 Logon ID。
    - 找出相同的 Logon ID 對應的登入事件。

練習：
1. 攻擊者在 RDP 登入後，創建了哪個使用者帳號？

    ![image](https://hackmd.io/_uploads/BkRQRHOlMl.png)
    篩選 4720 的事件後看到新的使用者帳號。
...
- Process info: 執行的行程的內容，包括它的 PID (Process ID)，執行檔的完整路徑 (image)，和程式執行的完整指令 (commandline)。
...
1. 打開 VM 桌面上的 `Practice-Sysmon.evtx` 檔案，Sarah 使用哪個瀏覽器來瀏覽網頁？
    ...
    ![image](https://hackmd.io/_uploads/SJgTtToxfx.png)
    
    A：`Google Chrome`
    
2. Sarah 從瀏覽器下載了哪個檔案？

    查找所有紀錄後，只有這個是在 Downloads 資料夾內的：
    ![image](https://hackmd.io/_uploads/Hy2snTilfx.png)
    
    或篩選 Event ID 為 11 的事件：
    ![image](https://hackmd.io/_uploads/rJtDmAieMg.png)
    
    A：`C:\Users\sarah.miller\Downloads\ckjg.exe`

3. 該檔案是從哪個網址上下載的？

    篩選 Event ID 為 15 的事件：
    ![image](https://hackmd.io/_uploads/BkcFzCslMe.png)

    A：`http://gettsveriff.com/bgj3/ckjg.exe`

> Event ID 1: 行程建立，當系統中有新的程式被啟動時，就會觸發 Event ID 1。

> Event ID 11: 檔案建立，當有任何檔案被建立或舊檔案被覆寫時，就會觸發 Event ID 11。
> `TargetFilename`: 被建立或修改的檔案完整路徑與名稱。
> `CreationUtcTime`: 檔案建立的時間。
> `Image`: 建立該檔案的程式。

> Event ID 15: 檔案資料流建立，用瀏覽器從網路下載檔案時，Windows 會自動在這個檔案的背後塞入一個隱藏的標記，叫做 `Zone.Identifier`，用來記錄這個檔案是從哪個網址下載來的。
> `TargetFilename`: 下載來的檔案路徑。
...
2. C2 (Command & Control) 伺服器惡意軟體連接到哪？

    篩選 event ID 3 的事件，可以看到唯一的事件中 ProcessId 一樣是 1460，接著尋找 DestinationIp 與 DestinationPort 欄位：
    ![image](https://hackmd.io/_uploads/rk3RPDCefg.png)
...
3. 這個惡意 IP 對應的網域是什麼？
    ...
    篩選 event ID 22 的事件：
    ![image](https://hackmd.io/_uploads/SywzswClfe.png)
    只有第一個事件的 QueryStatus 為 0，可能意為查詢成功，並且只有此事件有解析出 IP `::ffff:193.46.217.4;`，因此此事件的 QueryName 正是惡意 IP 的網域。
...
內建於 Windows 系統中的 PowerShell 經常被攻擊者利用，因為它受信任且能夠下載惡意軟體、系統偵查、資料外洩、甚至進階的技術如行程注入。然而僅透過查看行程建立的 logs 並不能捕捉到 PowerShell 的指令。

![image](https://hackmd.io/_uploads/H1jGghyZzg.png)
以此為範例，攻擊者成功讀取了敏感檔案，查看了 local users 與 local groups，並下載惡意軟體至 Temp 目錄中。然而這會有一個 event ID 1 事件，顯示 powershell.exe 行程啟動，但沒有執行的命令相關資訊。

每個程式都有其特定的用途，如 `firefox.exe` 是一個網頁瀏覽器，`notepad.exe` 是一個文字編輯器，`whoami.exe` 僅輸出使用者帳號名稱 (username)。如果只是要瀏覽網頁，可能只需要建立一個 firefox 行程，但如果要執行其它動作如 RDP 存取或照片編輯，就需要開啟新的行程並建立額外的日誌。

啟動 powershell.exe 後，可以在同一個終端會話中執行數百個指令，而無需為每個操作建立新的行程。

PowerShell History File：
有許多種方法可以監控 PowerShell，但在此僅先看一種簡單的方法，PowerShell 歷史紀錄檔案。
```
C:\Users\<USER>\AppData\Roaming\Microsoft\Windows\PowerShell\PSReadline\ConsoleHost_history.txt
```
由 PowerShell 自動建立的明文檔案，它會紀錄每個輸入並執行的指令。
![image](https://hackmd.io/_uploads/B1zVr31-fg.png)

練習：
1. 查看 VM 上管理員的 PowerShell 歷史，哪個 PowerShell 指令最先被執行？

    ![image](https://hackmd.io/_uploads/By2rnhyWfx.png)
    
    或
    ![image](https://hackmd.io/_uploads/By-vTnJWzl.png)
    
    A：`Get-ComputerInfo`
    
2. 管理員執行第一個指令的時間是什麼時候？
    
    在檔案總管中找到檔案，右鍵 -> 內容 (properties)：
    ![image](https://hackmd.io/_uploads/B16ap31-Mx.png)
